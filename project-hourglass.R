...
for (t_i in t) {
  if (t_i != 0) {
    ...
    png(paste("./project1-2/", filename, ".png", sep=""))
    ylim2=c(0, C_i)
    plot(x = 1:(n_box), y = conc_t_i[1:n_box], xlim = c(1, n_box), ylim = c(0, C_i), xlab = "#box", ylab = "concentration",axes=F)
    axis(side=1,at=seq(0,25))
    axis(side=2,pos=0,at=seq(0,5,5))
    points(x = 1:(n_box), y = conc_t_i[(n_box+1):(n_box*2)], xlim = c(n_box+1,n_box*2), ylim = rev(range(ylim2)), xlab = "#box", ylab = "concentration",col="red")
    dev.off()
  }
}
